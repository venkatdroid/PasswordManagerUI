...
export class SaveDataComponent {
  ...
  togglePassword() {
    ...
    if (this.showPassword && this.isEdit) {
      this.passwordValue = this.service.getDecryptedPasswordById(
        this.passwordDetail.id
      );
    }
  }

  onSubmit() {
    this.passwordDetail.password = this.passwordValue;
    if (
      this.passwordDetail.username &&
      this.passwordDetail.password &&
      this.passwordDetail.category &&
      this.passwordDetail.app
    ) 
    {
      if (this.isEdit) {
        this.service.updatePassword(this.passwordDetail).then((result) => {
          if (result) {
        ...
        this.service.createPassword(this.passwordDetail).then((result) => {
          if (result) {
            this.showDialog = true;
          }
        });
